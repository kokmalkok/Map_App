//
//  PlotInfoViewController.swift
//  Map App
//
// Created by Константин Малков on 24.11.2022.
//
...
class PlotInfoViewController: UIViewController {
    ...
    @objc private func didTapAddToFavourite(){
        ...
        if let coordinate = self.coordinatesForPlotInfo {
            ...
            SPAlert.present(message: "Error of saving.\nTry again later!", haptic: .error)
        }
    }
        ...
        geocoder.reverseGeocodeLocation(location) { [weak self] placemark, error in
            ...
            DispatchQueue.main.async {
                if let point = pointOfInterest, point != "" {
                    ...
                    self?.distanceLabelPlotView.text = "\(city), \(country)"
...
extension PlotInfoViewController: UITableViewDelegate, UITableViewDataSource {
