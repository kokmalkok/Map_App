...
class ResultViewController: UIViewController {
    ...
    override func viewDidLayoutSubviews(){
        ...
        table.frame = CGRect(x: 10, y: safeArea+110, width: view.frame.size.width-20, height: view.frame.size.height-80)
    ...
    private func alternativeParseData(customMark: MKPlacemark) -> String {
        ...
        let addressLine = "\(customMark.thoroughfare ?? ""), \(customMark.subThoroughfare ?? ""), \(customMark.subLocality ?? ""), \(customMark.administrativeArea ?? ""),  \(customMark.country ?? "")"
        return addressLine
    }
}
...
extension ResultViewController:  UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if segmentalButtons.selectedSegmentIndex == 0 {
            ...
            let placeDistance = convertDistance(user: self.userLocation!, annotation: selectedItems.coordinate)
            cell.textLabel?.text = (selectedItems.name ?? "Без имени") + "          \(placeDistance)"
            ...
            cell.textLabel?.text = convertData.nameCategory
